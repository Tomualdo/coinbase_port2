            ...
            if len(buys.keys())>=1:
                ...
                for idx in sells_not_empty_record:
                    ...
                    if not buys[idx][0]['sell_flag'] and actual_sell_price / buys[idx][0]['buy_price'] >= sell_price_ratio:
                        ...
                        push_note("SELL " + product," at "+(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                        sell_response = sell_market_size(product,base_min_size)
                        if sell_response['done_reason'] == 'filled':
                            # earnValue = buys[idx][0]['earn']
                            earnValue = float(sell_response['executed_value']) * float(sell_response['filled_size'])
            ...
            if len(buys.keys()) >= 1:
                ...
                if diff < set_minimum_buy_time:
                    ...
                    if last_buy_price / now_buy_price > buy_ratio:
                        ...
                        buy_response = order_market_size(product,base_min_size) #REAL
                        if buy_response['done_reason'] == 'filled':
                            ...
                            buys[time].append({'buy_time':time,
                                        ...
                                        'spend_EUR':float(buy_response['funds']) + float(buy_response['fill_fees']),
                                        'coins': coins,
                                        'fee': fee,
                                        'sell_flag' : False,
                                        'sell_price': 0,
                                        'sell_time' : 0,
                                        'earn':0}) #REAL
                            push_note("BUY " + product,str(coins)+" at "+(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    else:      
                        buy_signal.append(np.nan)
                        sell_signal.append(np.nan)
...
def product_tick(message):
    ...
    print (df_new)
    print ("LowBoll:{} UppBoll:{} STD:{} SMA:{} MinFunds:{} MaxBuys:{} MinSize:{}".format(strategy_lower_bolling_lvl,
